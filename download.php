<?php
	...
	$types =  $conn->prepare("SELECT * from `wp_lb_point_types`");
	$types->execute();
	$typesResult = $types->fetchAll();
	foreach($typesResult as $value):
		echo $typesArray[$value['id']]['type'] = $value['type'];
		echo $typesArray[$value['id']]['plusminus'] = $value['plusminus'];
	endforeach;

	//Get me all the dealers in an array:
	$dealers =  $conn->prepare("SELECT * from `wp_lb_dealers`");
	$dealers->execute();
	...
	$users =  $conn->prepare("SELECT * from `wp_users`");
	$users->execute();
	...
	// $rules =  $conn->prepare("SELECT * from `wp_point_rules`");
	// $rules->execute();
